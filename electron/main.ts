...
export async function fetchAllClickableItems(): Promise<ClickableItem[]> {
  if (process.platform === "win32") {
    if (windowsAccessibility) {
      ...
      // TODO: Implement actual Windows UIA integration for fetchAllClickableItems
      console.log(
        "fetchAllClickableItems (win32): Native UIA module not loaded. Returning empty array. Implement C# UIA component."
      );
      return []; // Placeholder
    }
    ...
    // This block should ideally not be reached if platforms are win32 or darwin
    console.warn(
      "fetchAllClickableItems: Truly unsupported platform:",
      process.platform
    );
    return [];
  }
...
ipcMain.on("message", async (event, msg) => {
  ...
  while (true) {
    ...
    ${clickableItemsText}`,
      ...
      if (stepsOutput?.type != "next_step_final_output") {
        console.warn("stepsOutput was not final, continuing loop.");
        continue;
      }
  ...
  ${
    structuredDOM
      ...
      if (scriptOutput?.type != "next_step_final_output") {
        console.warn("scriptOutput was not final, continuing loop.");
        continue;
      }
